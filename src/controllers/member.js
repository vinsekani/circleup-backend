...
const addMember = async (req, res) => {
  try {
    const { fullName, phone, groupId, startDate, userId } = req.body;

    if (!fullName || !phone || !groupId || !userId) {
      return res.status(400).json({ message: "Missing required fields" });
    }
    ...
    const existingMember = await Member.findOne({ user: userId, group: groupId });
    if (existingMember) {
...
const getGroupMembers = async (req, res) => {
  try {
    const { groupId } = req.params;
    const members = await Member.find({ group: groupId }).populate("user");
    return res.status(200).json(members);
  } catch (error) {
